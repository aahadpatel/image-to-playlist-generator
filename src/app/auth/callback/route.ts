...
const APP_URL = (process.env.NEXT_PUBLIC_APP_URL || "https://image-to-playlist-generator.vercel.app").trim();
...
export async function GET(request: NextRequest) {
  ...
  if (!code) {
    return NextResponse.redirect(new URL("/error?message=Missing_code", APP_URL));
  }
  ...
  try {
    ...
    redirectResponse.cookies.set("spotify_refresh_token", data.refresh_token, cookieOptions);

    return redirectResponse;
  } catch (error) {
    console.error("Error in callback:", error);
    return NextResponse.redirect(new URL("/error?message=Auth_failed", APP_URL));
  }
}